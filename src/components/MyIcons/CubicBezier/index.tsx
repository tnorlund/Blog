...
const linearInterpolate = ( x0: number, x1: number, t: number ) => ( 1.0 - t ) * x0 + t * x1
    ...
    >
      { ( props ) => (
        ...
        >
          ...
          <g id="P2"
            style={ { transform: translate( P2, height, width, letterDiff ) } }
            fill={fill}
          >
          ...
          <a.g id="P1"
            style={ {
              transform: translate( P1, height, width, letterDiff )
            } }
            fill={fill}
          >
          ...
          <g id="P3"
            style={ { transform: translate( P3, height, width, letterDiff ) } }
            fill={fill}>
            <path className="cls-1" d="M-4.83,3.4h1.72v.75c-.61-.05-2.16-.05-2.84-.05s-2.25,0-2.85.05V3.4h1.72V-6H-8.8v-.74h6.63c2.64,0,4.22,1.35,4.22,3.1S.2-.65-2-.65H-4.83ZM-.39-3.66C-.39-4.8-.39-6-2.77-6H-4.92V-1.3h2.14C-.39-1.3-.39-2.54-.39-3.66Z" />
          ...
          <a.circle
            ...
            cx={
              props.x
                ...
                .to( t => ( 1 - t )**3 * P0.x + 3 * ( 1 - t )**2 * t * P1.x + 3 * ( 1 - t ) * t**2 * P2.x + t**3 * P3.x )
            }
            cy={
              props.x
                .to( { range: [ min_x, max_x ], output: [ 0, 1 ] } )
                .to( t => ( 1 - t )**3 * P0.y + 3 * ( 1 - t )**2 * t * P1.y + 3 * ( 1 - t ) * t**2 * P2.y + t**3 * P3.y )
            }
          ...
          <a.line
            ...
            x1={props.x.to( { range: [ min_x, max_x ], output: [ P0.x, P1.x ] } )}
            y1={props.x.to( { range: [ min_x, max_x ], output: [ P0.y, P1.y ] } )}
            x2={props.x.to( { range: [ min_x, max_x ], output: [ P1.x, P2.x ] } )}
            y2={props.x.to( { range: [ min_x, max_x ], output: [ P1.y, P2.y ] } )}
          />
          <a.line
            style={{ opacity:0.5 }}
            stroke={red}
            strokeWidth={lineWidth}
            x1={props.x.to( { range: [ min_x, max_x ], output: [ P1.x, P2.x ] } )}
            y1={props.x.to( { range: [ min_x, max_x ], output: [ P1.y, P2.y ] } )}
            x2={props.x.to( { range: [ min_x, max_x ], output: [ P2.x, P3.x ] } )}
            y2={props.x.to( { range: [ min_x, max_x ], output: [ P2.y, P3.y ] } )}
          />

          <a.line
            stroke={red}
            strokeWidth={lineWidth}
            x1={props.x.to( { range: [ min_x, max_x ], output: [ 0, 1 ] } ).to( t => linearInterpolate( linearInterpolate( P0.x, P1.x, t ), linearInterpolate( P1.x, P2.x, t ), t ) ) }
            y1={props.x.to( { range: [ min_x, max_x ], output: [ 0, 1 ] } ).to( t => linearInterpolate( linearInterpolate( P0.y, P1.y, t ), linearInterpolate( P1.y, P2.y, t ), t ) ) }
            x2={props.x.to( { range: [ min_x, max_x ], output: [ 0, 1 ] } ).to( t => linearInterpolate( linearInterpolate( P1.x, P2.x, t ), linearInterpolate( P2.x, P3.x, t ), t ) ) }
            y2={props.x.to( { range: [ min_x, max_x ], output: [ 0, 1 ] } ).to( t => linearInterpolate( linearInterpolate( P1.y, P2.y, t ), linearInterpolate( P2.y, P3.y, t ), t ) ) }
          />
          <a.circle
            cx={props.x.to( { range: [ min_x, max_x ], output: [ 0, 1 ] } ).to( t => linearInterpolate( linearInterpolate( P0.x, P1.x, t ), linearInterpolate( P1.x, P2.x, t ), t ) )}
            cy={props.x.to( { range: [ min_x, max_x ], output: [ 0, 1 ] } ).to( t => linearInterpolate( linearInterpolate( P0.y, P1.y, t ), linearInterpolate( P1.y, P2.y, t ), t ) )}
            r={radius}
            fill={red}
          />
          <a.circle
            cx={props.x.to( { range: [ min_x, max_x ], output: [ 0, 1 ] } ).to( t => linearInterpolate( linearInterpolate( P1.x, P2.x, t ), linearInterpolate( P2.x, P3.x, t ), t ) ) }
            cy={props.x.to( { range: [ min_x, max_x ], output: [ 0, 1 ] } ).to( t => linearInterpolate( linearInterpolate( P1.y, P2.y, t ), linearInterpolate( P2.y, P3.y, t ), t ) ) }
            r={radius}
            fill={red}
          />
