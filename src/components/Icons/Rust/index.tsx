    ...
    <Spring to={ { x: inView ?  max_x : min_x } } >
      { ( props ) => (
        <svg width="100%" height="100%" viewBox="0 0 200 100" preserveAspectRatio="XmaxYMax">
          <a.path
            style={{
              opacity: props.x.to( [min_x, steps[0].start, steps[0].stop, max_x], [0, 0, 1, 1] ),
              position: `absolute`,
              transformOrigin: `23.8867% 50%`,
              transform: props.x.to( [min_x, steps[1].start, steps[1].stop, max_x], [1, 1, 0, 0] ).to( x => `rotate(${ 360 / 5 * x }deg)` )
            }}
            fill={fill}
