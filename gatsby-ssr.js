...
function setColorsByTheme() {
  // Don't use back-ticks around emojis. Breaks replacement in boundFn below.
  // eslint-disable-next-line quotes
  const [colors, colorModeKey, colorModeCssProp] = ['🌈', '🔑', '⚡️']
  // Default value if the user never used DarkToggle is to use the OS color
  // mode.
...
function RssSetColorsByTheme() {
  ...
  // Turn boundFn into an IIFE so it runs asap. Also avoids polluting global
  // namespace.
...
export const onRenderBody = ( { setPreBodyComponents, setHeadComponents } ) => {
  // Keys just to prevent warning: Each child in a list should have a unique
  // "key" prop.
