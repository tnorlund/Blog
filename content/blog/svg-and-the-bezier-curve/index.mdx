...
These graphics are fairly standard. There's `circle`, `ellipse`, `line` and `polyline`, but I'm more interested in `path`. This allows you to set *Bézier* and *elliptical* curves. These curves are encoded with specific letters to determine how the `path` moves in the `viewBox`.  
