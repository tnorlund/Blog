...
I spent a lot of time today trying to render a component through SSR, server-side rendering. I'm ending today defeated because the current version of ReactDOM does not support the new `<Suspense>` tag.
...
You can write everything in JavaScript the browser interprets, but this is 2021. Nobody got time for that. Babel converts this 'edge' JavaScript into plain ES6, allowing it to run in any browser.

<Babel/>

## Conclusion

The modern web is rendered on the server and on the client. Both use complex JavaScript, HTML, and CSS to render functional, scalable pages. My problem today was that I was rendering "client" JavaScript on the "server," skipping both Babel and Webpack.
