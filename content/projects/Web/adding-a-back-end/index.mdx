...
The S3 bucket acts as a data-lake for storing raw data. Another Lambda function is then used to transform the raw data into valuable information. I'm using [ipify](https://geo.ipify.org) to obtain location and proxy information from the raw IP address. This transformed data is then stored in another DynamoDB table.
