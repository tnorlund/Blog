...
Again, I've had no experience with DynamoDB. This book helped me tremendously, and after purchasing the book, you're invited to a group of other DynamoDB developers. DeBrie has an organized system for creating a Dynamo DB table.
...
The best way to go about the understanding the access patterns would be to list them by entity.
...
DynamoDB uses a single table to organize all the data. This means that the relationships, sorting, and access patterns use a combination of the partition key, *PK*, and the sort key, *SK*, to create, read, update, and delete the date on the database. Both the *PK* and *SK* make the primary key are used to get a specific row from the DynamoDB table. 

I found the best way to implement both the *PK* and *SK* was to create them while implementing the API's backend.
...
|                  | PK                     | SK                             |
|------------------|------------------------|--------------------------------|
| Project          | PROJECT#[Project Slug] | #PROJECT                       |
| Project Follow   | USER#[User Number]     | #PROJECT#[User Follow Number]  |

The Global Secondary Indexes are use to query the project's followers.
